...
try:
    ...
    packet = Ether(src=target_MAC, dst=server_MAC) / ARP(op=2, pdst=args.server_ip, psrc=args.target_ip, hwsrc=target_MAC,hwdst=server_MAC)
    sendp(packet, iface=interface)
    print(f"\nFixed server ARP table")
    packet = Ether(src=server_MAC, dst=target_MAC) / ARP(op=2, pdst=args.target_ip, psrc=args.server_ip, hwsrc=server_MAC, hwdst=target_MAC)
    sendp(packet, iface=interface)
